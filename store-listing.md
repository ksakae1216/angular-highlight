...
• AIの確信度が低い項目は表示しません
• 優先度: 低 / 中 / 高
• Jev APIに送信する情報（ONの間のみ）: 変更検知戦略（OnPush or Default）、再レンダリング回数、検知方式、親コンポーネントの有無、変更検知のきっかけ（mousemove や setInterval などイベント・APIの種類のみ）。コンポーネント名・ページの内容・URL・ユーザー入力は送信しません
